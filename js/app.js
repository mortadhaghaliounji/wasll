document.addEventListener("DOMContentLoaded", () => {
  ...
  mobileToggle.addEventListener("click", () => {
    // Le bouton du haut sert désormais de vrai toggle ouvrir/fermer :
    // avant, il ne faisait qu'ouvrir, et rien ne permettait de refermer
    // le panneau (l'overlay est masqué derrière le panneau plein écran).
  ...
  function renderCountries() {
    ...
    for (const [code, data] of Object.entries(ASSETS_DATA)) {
      ...
      btn.className =
        "country-btn" + (code === currentCountry ? " active" : "");
      btn.title = code;
  ...
  function renderStaircase() {
    ...
    steps.forEach((step, i) => {
      ...
      stepEl.className =
        "step" + (step.id === selectedStepId ? " selected" : "");
